...
public class GameSoundManager: MonoBehaviour
{
    ...
    private void HandleUpdateWaveAmbient(int stage)
    {
        ...
        switch (stage)
        {
            ...
            default: targetVolume = 0f; break; // 그 외 단계나 종료 시
        }

        // 볼륨 즉시 적용 (만약 서서히 커지게 하고 싶다면 이 부분에 DOTween이나 Coroutine을 사용하면 좋습니다)
        if (_rainSource != null) _rainSource.volume = targetVolume;
        if (_windSource != null) _windSource.volume = targetVolume;

        Debug.Log($"🌧️ [환경음] 웨이브 {stage}단계 진입! 비/바람 볼륨이 {targetVolume * 100}%로 조정되었습니다.");
    }

    // 🟢 번개 단발성 재생 로직
    private void HandlePlayLightning(float volume)
    {
        if (_lightningSource != null && _lightningSource.clip != null)
        {
            // PlayOneShot을 사용하면 번개가 연속으로 쳐도 소리가 씹히지 않고 겹쳐서 자연스럽게 들립니다.
            _lightningSource.PlayOneShot(_lightningSource.clip, volume);
            Debug.Log($"⚡ [번개] 콰쾅! (볼륨: {volume})");
        }
    }
