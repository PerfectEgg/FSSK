...
{
   [Header("기울임(Lean) 설정")]
    [SerializeField] private float leanDistance = 1.5f; // 좌우로 이동할 최대 거리
    [SerializeField] private float leanAngle = 10f;     // 좌우로 갸우뚱거릴 최대 각도(Z축)
    [SerializeField] private float leanSpeed = 8f;      // 기울어지는 속도 (부드러움 조절)
    ...
    private void Update()
    {
        ...
        Vector3 targetPos = initialLocalPos + new Vector3(h * leanDistance, 0, 0);
        ...
        float targetZRotation = -h * leanAngle;
        Quaternion targetRot = initialLocalRot * Quaternion.Euler(0, 0, targetZRotation);
