...
{
    // 지정된 레벨 프로세스
    private List<int> _levelProgression = new List<int> { 0, 1, 1, 1, 2, 2, 2, 3, 3, 3 };
    ...
    private void HandleWaveStage(int stage)
    {
        ...
        if (_currentLevel != newLevel)
        {
            ...
            ApplyWindBlowEffect(_currentLevel);
        }
    }

    private void ApplyWindBlowEffect(int level)
    {
        if (level == 0) return;

        Debug.Log($"[바람] {level}단계 번개 코루틴을 시작합니다!");
